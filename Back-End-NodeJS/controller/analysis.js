...
const createAnalysis = async (req, res) => {
  try {
    ...
    // const analysisCount = await Analysis.countDocuments({ owner: req.userId });
    await User.findOneAndUpdate({ _id: req.userId }, { $inc: { analysisCount: 1 } });
...
const saveAnalysis = async (req, res) => {
  ...
  try {
    ...
    await User.findOneAndUpdate({ _id: req.userId }, { $set: { savedAnalysis } });
